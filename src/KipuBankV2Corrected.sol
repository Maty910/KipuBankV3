...
contract KipuBankV2Corrected is KipuAccess, ReentrancyGuard {
    ...
    /// @notice Verifica que no se exceda el bankCap
    function _checkBankCap(uint256 ethAmount) private view {
    ...
    /// @notice Verifica que no se exceda el withdrawLimit
    function _checkWithdrawLimit(uint256 amount) private view {
        if (amount > WITHDRAW_LIMIT) {
            revert ExceedsWithdrawLimit(amount, WITHDRAW_LIMIT);
        }
    }
    ...
    function _updatePriceFromChainlink() private {
        ...
        // Casting seguro: Chainlink siempre devuelve valores positivos para precios
